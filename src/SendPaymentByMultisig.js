...
async function main(masterAddress, destinationAddress, regularKeys, amount, quorum, tags, memos) {
  try {
    const p = new Payment(masterAddress, regularKeys);

    const source = p.createSouce(amount, tags.source);
    const destination = p.createDestination(amount, destinationAddress, tags.destination);
...
main(masterAddress, destinationAddress, regularKeys, amount, quorum, tags, memos);
